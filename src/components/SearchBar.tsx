...
export default function SearchBar({
  ...
  return (
    <div className="relative flex w-full justify-end sm:justify-center" ref={containerRef}>
      ...
      <Button variant="ghost" size="icon" className="sm:hidden" onClick={() => setIsMobileOpen(true)}>
          <Search className="h-5 w-5" />
      </Button>

      {/* Search Input Container */}
      <div className={cn(
        "absolute left-0 top-0 w-full h-full sm:relative sm:w-auto sm:h-auto sm:opacity-100 sm:translate-y-0 transition-opacity duration-300",
         isMobileOpen ? 'opacity-100 z-20' : 'opacity-0 pointer-events-none sm:pointer-events-auto'
      )}>
        ...
        />
         <Button variant="ghost" size="icon" className="sm:hidden absolute right-1 top-1/2 -translate-y-1/2" onClick={() => setIsMobileOpen(false)}>
          <X className="h-5 w-5" />
        </Button>
      </div>
      ...
      {showResults && (
        <div className="absolute left-0 right-0 top-12 bg-card border rounded-lg shadow-lg max-h-96 overflow-auto z-50">
          ...
          {!loading && !error && results.length > 0 && (
              <ul>
                {results.map((r) => (
                    <li key={r.id} onClick={() => { onSelect?.(r); setIsFocused(false); setIsMobileOpen(false); }}
